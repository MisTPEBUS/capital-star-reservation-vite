...
export function Sidebar() {
  ...
  return (
    <aside className="border-b border-admin-border bg-admin-surface lg:sticky lg:top-0 lg:h-screen lg:overflow-y-auto lg:border-b-0 lg:border-r">
      <div className="flex min-h-14 items-center px-4 lg:h-16">
        <div>
      ...
      <nav className="flex gap-1 overflow-x-auto border-t border-admin-border px-2 py-2 lg:block lg:space-y-1 lg:border-t-0 lg:px-3 lg:py-3">
        {navigation.slice(0, 1).map((item) => (
          <NavLink
            key={item.to}
            className={({ isActive }) =>
              `shrink-0 rounded-adminControl px-3 py-2.5 text-sm font-semibold transition lg:block ${
                isActive
        ...
        <div className="shrink-0 lg:py-1">
          <button
            ...
            className={`flex w-full items-center justify-between gap-3 rounded-adminControl px-3 py-2.5 text-left text-sm font-semibold transition ${
              isDispatchActive
                ? "bg-admin-elevated text-adminStatus-enabled"
                : "text-admin-softText hover:bg-admin-elevated hover:text-admin-text"
            }`}
